...
function getArtistByIndex(arr, index) {
  for(let i = 0; i < arr.length; i++) {
    if(arr[i] === arr[index]) {
      return `the artist at index ${index} is ${arr[i].name}`
    }
  }
}
...
function get20s(arr) {
  const searchedCentury = [];
  for(let i = 0; i < arr.length; i++) {
    if(arr[i].years.split(' ', 1) >= 1900 && arr[i].years.split(' ', 1) <= 2000 ) {
      searchedCentury.push(arr[i].name);
    }
  }
  return searchedCentury;
}
...
function addArtist(arr) {
  const addedArtist = {
    'id': 20,
    'name': 'Victor Brambila', 
    'years': '1993 - 2022',
    'genre': 'Web Design', 
    'nationality': 'Mexican',
    'bio': 'Learning as I go'
  };
  arr.push(addedArtist);
  return arr;
}
